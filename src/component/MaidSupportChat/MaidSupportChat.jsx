...
import styles from "./MaidSupportChat.module.css";

const API_URL = import.meta.env.VITE_API_URL || "http://localhost:8080";
const SUPPORT_URL = `${API_URL}/api/maid-support-chat`;
const POLL_MS = 8000;
const MAX_FILES = 5;
...
// ─── Main MaidSupportChat component ─────────────────────────────────
// Props:
//   onClose — called when user taps ← back
export default function MaidSupportChat({ onClose }) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const res = await fetch(`${SUPPORT_URL}/conversation`, {
        headers: authH(),
      });
      if (!res.ok) throw new Error("Failed to load support chat");
      const data = await res.json();
      setConversation(data.conversation);
      setMessages(data.messages || []);
      if (data.conversation?.id) {
        fetch(`${SUPPORT_URL}/${data.conversation.id}/read`, {
          method: "PATCH",
          headers: authH(),
        }).catch(() => {});
      }
  ...
  async function handleSend() {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          const res = await fetch(
            `${SUPPORT_URL}/${conversation.id}/messages/media`,
            {
              method: "POST",
              headers: authH(),
              body: form,
    ...
    if (trimmed) {
      setSending(true);
      try {
        const res = await fetch(`${SUPPORT_URL}/${conversation.id}/messages`, {
          method: "POST",
  ...
  async function handleDelete(msgId) {
    try {
      await fetch(`${SUPPORT_URL}/messages/${msgId}`, {
        method: "DELETE",
        headers: authH(),
      });
    ...
    <div className={styles.root}>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerAvatar}>
          <span className={styles.headerAvatarInitial}>S</span>
          <span className={styles.onlineDot} />
        </div>
        <div className={styles.headerInfo}>
          <p className={styles.headerName}>Support Team</p>
          <p className={styles.headerSub}>We typically reply within minutes</p>
        </div>
      </div>
