...
import styles from "./AdminMaidSupportChats.module.css";
import { useNavigate } from "react-router-dom";

const API_URL = import.meta.env.VITE_API_URL || "http://localhost:8080";
const SUPPORT_URL = `${API_URL}/api/maid-support-chat/admin`;
  ...
  return (
    <div
      className={`${styles.bubbleWrap} ${
        isMaid ? styles.bubbleWrapLeft : styles.bubbleWrapRight
      }`}
    >
      {isMaid && (
        <div className={styles.bubbleAvatar}>
          <Avatar
            name={conversation.maid_name}
...
function ConversationDetail({ conversationId, onBack }) {
  ...
  const REPLY_URL = `${API_URL}/api/maid-support-chat`;
        ...
        <div className={styles.detailCard}>
          ...
          <div className={styles.replyArea}>
            ...
            <p className={styles.replyHint}>
              🛡️ Replying as Support Team · Maid will see your message
              immediately
            </p>
          </div>
  ...
  return (
    ...
    >
      <div className={styles.convRowAvatar}>
        <Avatar name={conv.maid_name} src={conv.maid_avatar} size={42} />
        {hasUnread && <span className={styles.unreadDot} />}
      </div>

      <div className={styles.convRowBody}>
        <div className={styles.convRowTop}>
          <span className={styles.convNames}>{conv.maid_name}</span>
        ...
        {conv.maid_email && (
          <p className={styles.convEmail}>{conv.maid_email}</p>
        )}
...
// ─── Main AdminMaidSupportChats ──────────────────────────────────────
export default function AdminMaidSupportChats() {
      ...
      <div className={styles.listHeader}>
        <div>
          ...
          <p className={styles.pageSub}>
            Reply to maid support messages as Support Team
          </p>
        </div>
          ...
          <p className={styles.emptyTitle}>
            {showDeleted
              ...
              : search
                ...
                : "No maid support conversations yet"}
          </p>
          <p className={styles.emptySub}>
            {showDeleted
              ? "No maids have deleted any chats yet."
              : search
                ? "Try searching by name or email."
                : "Maid support conversations will appear here."}
          </p>
        </div>
      ) : (
        <>
