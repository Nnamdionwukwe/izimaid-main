...
import styles from "./AdminMaidSupport.module.css";
...
function getAdminSeen(ticketId) {
  return parseInt(
    sessionStorage.getItem(`admin_maid_seen_${ticketId}`) || "0",
    10,
  );
}
function setAdminSeen(ticketId, n) {
  sessionStorage.setItem(`admin_maid_seen_${ticketId}`, String(n));
}
    ...
    >
      ...
      {unread > 0 && (
        <div className={styles.rowNewMsg}>
          🔔 {unread} new maid message{unread > 1 ? "s" : ""}
        </div>
      )}
    </button>
  );
          ...
          {isOpen ? (
            <div className={styles.replyArea}>
              <textarea
                ...
                placeholder="Type your reply to the maid…"
                rows={3}
                onKeyDown={(e) => {
...
// ─── Main ───────────────────────────────────────────────────────────
export default function AdminMaidSupport({ onBack }) {
        ...
        <div className={styles.headerLeft}>
          ...
          <div>
            <div className={styles.titleRow}>
              <h1 className={styles.pageTitle}>Maid Support Tickets</h1>
              {totalUnread > 0 && (
