...
import styles from "./AdminShelter.module.css";
...
const STATUS_OPTIONS = [
  ...
  { value: "active", label: "Active Partner", color: "#c9a84c", icon: "🤝" },
];
...
export default function AdminShelter() {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      if (filters.supportType)
        params.append("supportType", filters.supportType);
      params.append("page", filters.page);
      params.append("limit", filters.limit);

      const response = await axios.get(
        `${API_BASE_URL}/api/shelter/applications?${params}`,
  ...
  const fetchStats = useCallback(async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${API_BASE_URL}/api/shelter/applications/stats`,
        { headers: { Authorization: `Bearer ${token}` } },
      );

      if (response.data.success) {
        setStats(response.data.stats);
        setSupportTypeStats(response.data.supportTypeStats || []);
  ...
  const updateStatus = async (id, status, notes = null) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${API_BASE_URL}/api/shelter/applications/${id}/status`,
        { status, notes },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const updateNotes = async (id, notes) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.patch(
        `${API_BASE_URL}/api/shelter/applications/${id}/notes`,
        { notes },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const deleteApplication = async (id) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${API_BASE_URL}/api/shelter/applications/${id}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const bulkUpdateStatus = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/shelter/applications/bulk/status`,
  ...
  const exportToCSV = () => {
    ...
    const rows = applications.map((app) => [
      ...
      app.phone,
      app.city,
      app.organisation_type || "N/A",
      app.support_type,
      app.resident_count || "N/A",
      app.status,
    ...
    a.href = url;
    a.download = `shelter-applications-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    return (
      app.organisation_name.toLowerCase().includes(search) ||
      ...
      {filteredApplications.map((app) => (
        <div key={app.id} className={styles.mobileCard}>
          ...
          <div className={styles.mobileCardBody}>
            <div className={styles.mobileCardName}>{app.organisation_name}</div>
            <div className={styles.mobileCardDetails}>
              <div>📧 {app.email}</div>
              <div>📞 {app.phone}</div>
              <div>📍 {app.city}</div>
              <div>📋 {app.support_type}</div>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          <div className={styles.headerLeft}>
            <h1 className={styles.title}>Shelter & Agency Applications</h1>
            <p className={styles.subtitle}>
              Manage and review all partnership applications
            </p>
          </div>
        ...
        {stats && (
          <>
            ...
            {supportTypeStats.length > 0 && (
              <div className={styles.supportTypeStats}>
                <p className={styles.supportTypeStatsTitle}>By Support Type</p>
                <div className={styles.supportTypeStatsGrid}>
                  {supportTypeStats.slice(0, 5).map((item, i) => (
                    <div key={i} className={styles.supportTypeStatItem}>
                      <span className={styles.supportTypeStatLabel}>
                        {item.support_type}
                      </span>
                      <span className={styles.supportTypeStatCount}>
                        {item.count}
                      </span>
                    </div>
                  ))}
        ...
        <div className={styles.filtersBar}>
          <div className={styles.searchWrapper}>
            ...
            <input
              ...
              placeholder="Search by organisation, contact, email or reference..."
              value={searchTerm}
          ...
          <div className={styles.filterGroup}>
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, supportType: e.target.value, page: 1 })
              }
            >
                  ...
                  <tbody>
                    {filteredApplications.map((app) => (
                      <tr key={app.id} className={styles.tableRow}>
                        ...
                        <td className={styles.name}>{app.organisation_name}</td>
                        <td>
                        ...
                        <td>{app.city}</td>
                        <td className={styles.supportType}>
                          {app.support_type}
                        </td>
                        <td>
                          <span
                ...
                <div className={styles.detailsSection}>
                  <div className={styles.detailsGrid}>
                    ...
                    <div className={styles.detailItem}>
                      ...
                      <p>{selectedApplication.organisation_name}</p>
                    </div>
                    <div className={styles.detailItem}>
                    ...
                    <div className={styles.detailItem}>
                      ...
                      <p>
                        {selectedApplication.organisation_type ||
                          "Not specified"}
                      </p>
                    </div>
                    <div className={styles.detailItem}>
                      <label>Support Type</label>
                      <p>{selectedApplication.support_type}</p>
                    </div>
                    <div className={styles.detailItem}>
                      <label>Residents/Beneficiaries</label>
                      <p>
                        {selectedApplication.resident_count || "Not specified"}
                      </p>
                    </div>
                    <div className={styles.detailItem}>
                      <label>Status</label>
