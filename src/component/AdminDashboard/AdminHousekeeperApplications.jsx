// AdminHousekeeperApplications.jsx
import { useState, useEffect, useCallback } from "react";
import styles from "./AdminHousekeeperApplications.module.css";
...
export default function AdminHousekeeperApplications({ onBack }) {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/housekeeper-training/applications?${params}`,
  ...
  const fetchStats = useCallback(async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${API_BASE_URL}/api/housekeeper-training/applications/stats`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const updateStatus = async (id, status, notes = null) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${API_BASE_URL}/api/housekeeper-training/applications/${id}/status`,
        { status, notes },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const updateNotes = async (id, notes) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.patch(
        `${API_BASE_URL}/api/housekeeper-training/applications/${id}/notes`,
        { notes },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const deleteApplication = async (id) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${API_BASE_URL}/api/housekeeper-training/applications/${id}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const bulkUpdateStatus = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/housekeeper-training/applications/bulk/status`,
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `housekeeper-applications-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
      ...
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          <div className={styles.headerLeft}>
            <h1 className={styles.title}>Housekeeper Applications</h1>
            <p className={styles.subtitle}>
              Manage and review all training applications
            </p>
          </div>
