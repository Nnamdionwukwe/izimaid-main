// AdminCaregiverApplications.jsx
import { useState, useEffect, useCallback } from "react";
import styles from "./AdminCaregiverApplications.module.css";
...
export default function AdminCaregiverApplications({ onBack }) {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/caregiver-training/applications?${params}`,
  ...
  const fetchStats = useCallback(async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `${API_BASE_URL}/api/caregiver-training/applications/stats`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const updateStatus = async (id, status, notes = null) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${API_BASE_URL}/api/caregiver-training/applications/${id}/status`,
        { status, notes },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const updateNotes = async (id, notes) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.patch(
        `${API_BASE_URL}/api/caregiver-training/applications/${id}/notes`,
        { notes },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const deleteApplication = async (id) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${API_BASE_URL}/api/caregiver-training/applications/${id}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const bulkUpdateStatus = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/caregiver-training/applications/bulk/status`,
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `caregiver-applications-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
      ...
      {filteredApplications.map((app) => (
        <div key={app.id} className={styles.mobileCard}>
          ...
          <div className={styles.mobileCardBody}>
            ...
            <div className={styles.mobileCardCourse}>
              {app.preferred_course}
            </div>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          <div className={styles.headerLeft}>
            <h1 className={styles.title}>Caregiver Training Applications</h1>
            <p className={styles.subtitle}>
              Manage and review all caregiver training applications
            </p>
          </div>
                  ...
                  <tbody>
                    {filteredApplications.map((app) => (
                      <tr key={app.id} className={styles.tableRow}>
                        ...
                        <td>{app.city}</td>
                        <td className={styles.track}>{app.preferred_course}</td>
                ...
                <div className={styles.detailsSection}>
                  <div className={styles.detailsGrid}>
                    ...
                    <div className={styles.detailItem}>
                      ...
                      <p>{selectedApplication.preferred_course}</p>
                    </div>
                    <div className={styles.detailItem}>
